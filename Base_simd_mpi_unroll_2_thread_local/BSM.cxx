...
compare the result with the analytical equation of Black-Scholes Merton : more
...
// Function to calculate the Black-Scholes call option price using Monte Carlo
// method
double black_scholes_monte_carlo(ui64 S0, ui64 K, double T, double r,
  ...
  float64x2_t drift =
      vmulq_f64(vsubq_f64(vsubq_f64(r_vec, q_vec),
  ...
  for (ui64 i = 0; i < num_simulations; i += 4) {
    ...
    float64x2_t ST1 =
        vmulq_f64(S0_vec, float64x2_t{exp(vgetq_lane_f64(exponent1, 0)),
                                      exp(vgetq_lane_f64(exponent1, 1))});
    float64x2_t ST2 =
        vmulq_f64(S0_vec, float64x2_t{exp(vgetq_lane_f64(exponent2, 0)),
                                      exp(vgetq_lane_f64(exponent2, 1))});

    float64x2_t payoff = vaddq_f64(vmaxq_f64(vsubq_f64(ST1, K_vec), zero),
...
int main(int argc, char *argv[]) {
  ...
  for (ui64 run = 0; run < runs_per_process; ++run) {
    local_sum +=
        black_scholes_monte_carlo(S0, K, T, r, sigma, q, num_simulations);
  }
  ...
  if (rank == 0)
    std::cout << std::fixed << std::setprecision(6)
              << " value= " << global_sum / num_runs << " in "
              << (t2 - t1) / 1000000.0 << " seconds" << std::endl;
  MPI_Finalize();
  return 0;
}
