...
double black_scholes_monte_carlo(float64x2_t S0_vec, float64x2_t K_vec, float64x2_t sub_diffusion, float64x2_t drift, ui64 num_simulations) {
...
int main(int argc, char* argv[]) {
    ...
    double rt_exp = exp( -r * T);
    for (ui64 run = 0; run < num_runs; ++run) {
        sum+= rt_exp * black_scholes_monte_carlo(S0_vec, K_vec, sub_diffusion, drift, num_simulations);
    }
